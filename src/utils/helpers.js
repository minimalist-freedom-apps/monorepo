...
export const formatBtcWithCommas = (value) => {
...
export const formatFiatWithCommas = (value) => {
    if (!value || isNaN(value)) return '0';
...
export const formatSats = (sats) => {
    if (!sats || isNaN(sats)) return '0';

    const num = parseFloat(sats);
...
export const isLongerThan1Hour = (timestamp) => {
    const now = Date.now();
    const diff = now - timestamp;
