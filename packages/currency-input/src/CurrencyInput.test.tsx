...
describe(CurrencyInputPure.name, () => {
    ...
    test('calls onChange with value change', () => {
        ...
        assert.deepStrictEqual(onChange.mock.calls.at(-1)?.arguments, [200_000_000]);
    });
    ...
    test('switch SATS -> BTC & BTC -> SATS reformats focused BTC input', () => {
        ...
        assert.ok(document.body.contains(screen.getByDisplayValue('123,456,789.12,345,678')));
    ...
    [
        ...
        test(it, () => {
            ...
            assert.strictEqual((input as HTMLInputElement).value, initial.value);
            fireEvent.change(input, {
                target: {
                    value: change.value,
            ...
            assert.strictEqual((input as HTMLInputElement).value, final.value);
